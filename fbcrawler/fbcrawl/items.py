...
def reactions_strip(string, loader_context):
    ...
    if lang == 'it':
        ...
        elif newstring.replace("người khác", "").split()[::-1][0].isdigit():
            friends = newstring.count(' và ') + newstring.count(',')
            newstring = newstring.replace("người khác", "").split()[::-1][0]
            while newstring.rfind(',') != -1:
                newstring = newstring[0:newstring.rfind(',')] + newstring[newstring.rfind(',') + 1:]
            return int(newstring) + friends
    ...
    # =============================================================================
        ...
        # l = 2
            ...
            if date[1] == 'min' or date[1] == 'mins':
                if int(str(datetime.now().time()).split(sep=':')[1]) - int(date[0]) < 0 and int(
                        str(datetime.now().time()).split(sep=':')[0]) == 0:
                    day = int(str(datetime.now().date() - timedelta(1)).split(sep='-')[2])
                    month = int(str(datetime.now().date() - timedelta(1)).split(sep='-')[1])
                    return datetime(year, month, day).date()
                # 22 min (oggi)
...
class FbcrawlItem(scrapy.Item):
    campaign = scrapy.Field(output_processor=TakeFirst())
...
class CommentsItem(scrapy.Item):
    source = scrapy.Field(output_processor=TakeFirst())
    reply_to = scrapy.Field(output_processor=TakeFirst())
